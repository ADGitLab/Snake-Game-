...
let timeForBlueFood = 3000; // Time for blue food to appear in ms
let timer = 0; // Timer to track when blue food should appear
let map = null;
let foodType = 3;
let snakeSpeed = 200; // Normal speed
...
let walls = [];
let wallGenerationTime = 10000; // 1 minutes before walls start
let wallGenerationInterval = 6000; // 30 seconds between wall generations
...
function slowDownSpeed() {
  ...
  speedTimeout = setTimeout(() => {
    snakeSpeed = 200; // Reset to normal speed after 5 seconds
    setTimeout(() => {
      snakeSpeed = 100;
    }, 60000);
...
function moveSnake() {
  ...
  if (head.x === food.x && head.y === food.y) {
    if (foodType === 1) {
      ...
      poin.textContent = score += 5;
    } else if (foodType === 3) {
      poin.textContent = score += 15;
    }
